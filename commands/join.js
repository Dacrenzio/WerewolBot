module.exports = {
	...
	execute(message, args, moderatore){
		...
		if(moderatore.playerList.size +1 <= moderatore.playerNum){ //if there's space in the lobby
			
			//if the player is alredy in the game don't add
			if(moderatore.playerList.has(message.member)){
				embed.sendEmbed([255,0,0], "Giocatore gia presente.", message.channel);
				return;
			}
			...
			embed.sendEmbed([255,0,0], "Raggiunto il numero massimo di giocatori!", message.channel);
		}
	}
}