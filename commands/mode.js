module.exports = {
	...
	execute(message,args,mod){
		...
		if(args.length !== 1){
			...
			embed.sendEmbed([149, 193, 255], 'Modalità attuale: '+ modali, message.channel);
			return;
		}
		...
		if(args[0].valueOf() === 'auto'){
			...
			embed.sendEmbed([149, 193, 255], "Modalità auto attivata", message.channel);
		}else if(args[0].valueOf() === 'manual'){
			message.member.roles.add(role).catch(console.error);
			embed.sendEmbed([149, 193, 255], "Modalità auto disattivata", message.channel);
