			...
			for (var i = 0; i < mentionedArray.length; i += 1){
				...
				if(called.tratto.includes('protetto') || caller.tratto.includes('pazzo')){
					...
					if(called.tratto.includes('eroe')){
						...
						moderatore.playerList.get(mentioned).tratto.splice(indexEroe+1, 0, message.member);
					}
...
function amato(moderatore, called, mentioned, embed){
	...
	if(called.tratto.includes('amato') && moderatore.playerList.get(angelo).alive){
...
function discendente(moderatore, mentioned, embed, message){
	...
	moderatore.playerList.get(mentioned).tratto.splice(called.tratto.indexOf('discendente'), 1);
	embed.sendEmbed([149,193,255], `${mentioned.toString()} era un discendente dei lupi, dategli il benvenuto!`, message.channel);
	...
	embed.sendEmbed([149,193,255], "Eri un discendente dei lupi, ecco i tuoi nuovi compagni:\n\n"+lupi, mentioned);
